-- V167 — Add rolling THIS_MONTH forecast window for testing and planning
--
...
WITH active_locations AS (
    ...
    SELECT
        ...
        CASE
            ...
            WHEN p.projected_empty_at < now() + interval '7 days' THEN 'THIS_WEEK'
            WHEN p.projected_empty_at < now() + interval '14 days' THEN 'NEXT_WEEK'
            WHEN p.projected_empty_at < now() + interval '30 days' THEN 'THIS_MONTH'
            ELSE 'LATER'
    ...
    UNION
    SELECT customer_id, customer_address_id, product_id FROM forecast_signal WHERE forecast_window IN ('TODAY','TOMORROW','THIS_WEEK','NEXT_WEEK','THIS_MONTH')
)
SELECT
    dk.customer_id,
    loc.customer_name,
    ...
    CASE
        WHEN fc.forecast_window IN ('TODAY','TOMORROW','THIS_WEEK','NEXT_WEEK','THIS_MONTH')
         AND NOT (
    ...
    CASE
        ...
        WHEN fc.forecast_window = 'THIS_MONTH' THEN 'FORECAST_THIS_MONTH'
        ELSE 'NO_DEMAND'
    ...
    CASE
        ...
        AND fc.forecast_window IN ('TODAY','TOMORROW','THIS_WEEK','NEXT_WEEK','THIS_MONTH')
            THEN 'MIXED'
        WHEN (
            (sp.spot_checked_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR sp.spot_checked_at >= fc.latest_delivered_at))
            OR (ord.latest_order_requested_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR ord.latest_order_requested_at >= fc.latest_delivered_at))
    ...
    CASE
        ...
        WHEN fc.forecast_window = 'THIS_MONTH' THEN '#84cc16'
        ELSE '#16a34a'
    END AS marker_color,
    CASE
        ...
        WHEN fc.forecast_window = 'THIS_MONTH' THEN 8
        ELSE 99
    ...
    GREATEST(
        ...
        CASE WHEN ph.phone_verified_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR ph.phone_verified_at >= fc.latest_delivered_at) THEN COALESCE(ph.phone_verified_empty_count, 0) ELSE 0 END,
        CASE WHEN ord.latest_order_requested_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR ord.latest_order_requested_at >= fc.latest_delivered_at) THEN COALESCE(ord.order_requested_cylinder_count, 0) ELSE 0 END,
        CASE WHEN fc.forecast_window IN ('TODAY','TOMORROW','THIS_WEEK','NEXT_WEEK','THIS_MONTH')
    ...
    CASE
        WHEN fc.forecast_window NOT IN ('TODAY','TOMORROW','THIS_WEEK','NEXT_WEEK','THIS_MONTH') THEN FALSE
        WHEN (
            (ph.phone_verified_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR ph.phone_verified_at >= fc.latest_delivered_at))
            OR (sp.spot_checked_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR sp.spot_checked_at >= fc.latest_delivered_at))
            OR (ord.latest_order_requested_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR ord.latest_order_requested_at >= fc.latest_delivered_at))
    ...
    CASE
        WHEN fc.forecast_window NOT IN ('TODAY','TOMORROW','THIS_WEEK','NEXT_WEEK','THIS_MONTH') THEN 'NO_FORECAST'
        WHEN (
            (ph.phone_verified_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR ph.phone_verified_at >= fc.latest_delivered_at))
            OR (sp.spot_checked_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR sp.spot_checked_at >= fc.latest_delivered_at))
            OR (ord.latest_order_requested_at IS NOT NULL AND (fc.latest_delivered_at IS NULL OR ord.latest_order_requested_at >= fc.latest_delivered_at))
...
'Unified quantity-based delivery-planning signals with rolling TODAY, TOMORROW, THIS_WEEK (under 7 days), NEXT_WEEK (under 14 days) and THIS_MONTH (under 30 days) windows.';

CREATE OR REPLACE VIEW public.vw_delivery_planning_forecast_confirmation_worklist AS
SELECT
    s.*,
    CASE s.confirmation_status
        WHEN 'UNCONFIRMED' THEN 1
        WHEN 'CONFIRMED' THEN 2
        WHEN 'STALE_CONFIRMATION' THEN 3
        ELSE 4
    END AS confirmation_queue_order,
    CASE s.forecast_window
        WHEN 'TODAY' THEN 1
        WHEN 'TOMORROW' THEN 2
        WHEN 'THIS_WEEK' THEN 3
        WHEN 'NEXT_WEEK' THEN 4
        WHEN 'THIS_MONTH' THEN 5
        ELSE 9
    END AS forecast_window_order
FROM public.vw_customer_delivery_planning_signal s
WHERE s.forecast_window IN ('TODAY','TOMORROW','THIS_WEEK','NEXT_WEEK','THIS_MONTH');

COMMENT ON VIEW public.vw_delivery_planning_forecast_confirmation_worklist IS
'Forecast confirmation queue including THIS_MONTH. Unconfirmed first, confirmed next, stale confirmations last.';
