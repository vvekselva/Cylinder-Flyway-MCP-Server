-- V163 — Preserve mixed deterministic and forecast delivery-planning demand
--
-- V162 introduced independent deterministic_demand_score and
-- non_deterministic_demand_score columns, but demand_signal_type still
-- classified a row with both signal types as DETERMINISTIC. Some application
-- queries then derived metrics from that single classification, suppressing
-- the forecast contribution. This migration labels such rows MIXED while
-- retaining both independent score columns.
...
'Unified customer delivery-planning demand signals. Deterministic and forecast scores are independent; rows containing both are classified as MIXED.';
